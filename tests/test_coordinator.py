"""Tests for the PostNord coordinator: fetching, caching, events and the
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["statusText"] = {"header": "In transit"}
    return sample
...
async def test_polling_is_hot_for_an_out_for_delivery_parcel(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.return_value = active_sample()  # status: "DELIVERY"
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = PostNordApiError("HTTP 500")
...
async def test_fires_delivery_time_changed_event(hass):
    ...
    moved = active_sample()
    moved["estimatedTimeOfArrival"] = "2026-04-29T16:00:00Z"
...
async def test_losing_the_eta_is_silent(hass):
    ...
    dropped["estimatedTimeOfArrival"] = None
