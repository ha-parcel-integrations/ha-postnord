# Automatic parcel tracking from e-mail (IMAP → PostNord)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the PostNord tracking code, and register it with `postnord.track_parcel` — fully automatic, no extra custom component required.

PostNord is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `postnord.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ postnord.track_parcel
                                     │
...
- This integration, with the `postnord.track_parcel` action available (field `tracking_code`).
...
The regex matches a run of 12 or more digits, not embedded in a longer token:

```
(?<![0-9A-Za-z])[0-9]{12,}(?![0-9A-Za-z])
```

PostNord parcel numbers are long digit strings (commonly 12-20 digits) with no consistent letter prefix, so the example anchors on the **digit run** rather than a prefix. The integration itself accepts any code matching `^[A-Z0-9]{6,30}$`, but matching *that* broadly against e-mail text would hit every order number in the mail. A long-digit match is a good balance, but it can still pick up an equally long order or invoice number — so **scope the IMAP search to PostNord's sender** (see [Scope & privacy](#scope--privacy)), and let the AI fallback handle the rarer alphanumeric code formats.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@postnordshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: 12345678901234567","initial":true,"folder":"INBOX","username":"test"}'
```

Then `postnord.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
