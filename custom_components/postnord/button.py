"""Button platform for the PostNord parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the PostNord refresh button from a config entry."""
    async_add_entities([PostNordRefreshButton(entry)])


class PostNordRefreshButton(ButtonEntity):
    """Button that forces an immediate poll of all tracked PostNord parcels."""

    _attr_has_entity_name = True
    _attr_translation_key = "refresh"
    _attr_attribution = ATTRIBUTION

    def __init__(self, entry: PostNordConfigEntry) -> None:
