"""Sensor platform for the PostNord parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up PostNord sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        PostNordIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        PostNordAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            PostNordParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
...
class PostNordIncomingParcelsSensor(
    CoordinatorEntity[PostNordCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                PostNordParcelSensor(self.coordinator, self._entry, barcode)
...
class PostNordAwaitingPickupSensor(
    CoordinatorEntity[PostNordCoordinator], SensorEntity
):
    ...
    def __init__(self, coordinator: PostNordCoordinator, entry: ConfigEntry) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class PostNordParcelSensor(CoordinatorEntity[PostNordCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked PostNord parcel."""
    ...
    def __init__(
        self, coordinator: PostNordCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class PostNordNextDeliverySensor(
    CoordinatorEntity[PostNordCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: PostNordCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class PostNordDeliveredParcelsSensor(
    CoordinatorEntity[PostNordCoordinator], SensorEntity
):
    """Recently delivered tracked PostNord parcels."""
...
class PostNordLastUpdateSensor(
    ...
    """Diagnostic sensor reporting when PostNord was last polled successfully."""
