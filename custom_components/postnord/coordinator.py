"""Coordinator for the PostNord parcel tracker integration.
...
class PostNordCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (PostNordApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                _LOGGER.warning("PostNord fetch failed for %s: %s", code, result)
            ...
            if result is None:
                ...
                raws.append(self._raw_cache.get(code) or {"shipmentId": code})
                continue

            # The response's own shipment id can be missing on edge payloads;
            # fall back to the code we asked for so the sensor keeps its key.
            result.setdefault("shipmentId", code)
            self._raw_cache[code] = result
            raws.append(result)

        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("PostNord unreachable for all tracked parcels")
