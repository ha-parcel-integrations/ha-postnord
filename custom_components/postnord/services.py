"""Services for the PostNord parcel tracker integration.

`postnord.track_parcel` / `postnord.untrack_parcel` let you add or remove a
...
def async_setup_services(hass: HomeAssistant) -> None:
    """Register the PostNord services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid PostNord tracking code"
            )
